...
import "../css/routes/adminModule.css";
...
function AdminModule() {
    ...
    return ( <div className = "admin-module" >
        ...
        <h4 > LinkedIn Profile </h4> <input type = "url"
        ...
        { /* Email Management */ } <div className = "email-management" >
        <
        h4 className = "black" > Emails </h4> {
            companyForm.emails.map((email, index) => ( <div key = {
                    index
                }
                ...
                required = {
                    ...
                    companyForm.emails.length > 1 && ( <button type = "button"
                        onClick = {
                            () =>
                            setCompanyForm((prev) => ({
                                ...prev,
        ...
        { /* Phone Number Management */ } <div className = "phone-management" >
        <
        h4 className = "black" > Phone Numbers </h4> {
            companyForm.phoneNumbers.map((phone, index) => ( <div key = {
                    index
                }
                ...
                > {
                    phoneErrors[index] && ( <p className = "error-message" > {
                            phoneErrors[index]
                        } </p>
                    )
                } {
                    companyForm.phoneNumbers.length > 1 && ( <button type = "button"
                        onClick = {
                            () => {
                                setCompanyForm((prev) => ({
                                    ...prev,
        ...
        onChange = {
         ...
         <button type = "submit"
        disabled = {
            isAddCompanyDisabled
        } > {
            editingCompany ? "Update Company" : "Add Company"
        } </button> {
            editingCompany && ( <button type = "button"
                onClick = {
                    () => setEditingCompany(false)
                } >
                Cancel </button>
        ...
        <section className="communication-method-management">
            <h2>{editingMethod ? "Edit Communication Method" : "Add Communication Method"}</h2>
                ...
                {state.communicationMethods.map((method) => (
                    <div key={method.id} className="method-item">
                        ...
                        <div className="method-actions">
                            ...
                            <button onClick={() => deleteCommunicationMethod(method.id)}>Delete</button>
                        </div>
                    </div>
                ))}
            </div>
