...
import "../css/routes/companies.css";
    ...
    const renderContactDetails = (company) => {
        return ( <div className = "contact-details" >
            ...
            <h4 className = "black" > Emails </h4> {
                company.emails && company.emails.length > 0 ? ( <ul > {
                        company.emails.map((email, index) => ( <li key = {
                                index
                            } >
                            <span className = "email-icon" > ✉️ </span> {email} </li>
                        ))
                    } </ul>
            ...
            <h4 className = "black" > Phone Numbers </h4> {
                company.phoneNumbers && company.phoneNumbers.length > 0 ? ( <ul > {
                        company.phoneNumbers.map((phone, index) => ( <li key = {
                                index
                            } >
                            <span className = "phone-icon" > 📞 </span> {phone} </li>
                        ))
                    } </ul>
                ...
                <div key={company.id} className="company-card">
                    {editingCompany === company.id ? (
                        ...
                        <div className="company-details">
                            ...
                            <p>Communication Frequency: {company.communicationPeriodicity} days</p>
                            {renderContactDetails(company)}
                            <div className="company-actions">
                                <button onClick={() => startEditCompany(company)}>Edit Company</button>
